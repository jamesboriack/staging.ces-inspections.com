...
// Version: summary-2025-10-09-v8a (regex-free getCookie)

(function(){
  'use strict';
  const VERSION = 'summary-2025-10-09-v8a';
  ...
  // Robust, regex-free cookie reader (prevents parse errors)
  function getCookie(name){
    const all = typeof document !== 'undefined' ? document.cookie : '';
    if (!all) return '';
    const rows = all.split(/;\s*/);
    for (const row of rows){
      const i = row.indexOf('=');
      if (i === -1) continue;
      const k = row.slice(0, i);
      if (k === name) return decodeURIComponent(row.slice(i+1));
    }
    return '';
  }
  ...
  function firstUrlByKeyPart(partList){
    try {
      for (let i=0;i<localStorage.length;i++){
        ...
        const v = localStorage.getItem(kRaw) || '';
        if (!v) continue;
